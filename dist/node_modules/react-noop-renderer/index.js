(function (global, factory) {
	...
	const PassiveEffect = 0b0001000; // 当前fiber本次更新存在副作用
	...
	const mergeLanes = (laneA, laneB) => {
	    return laneA | laneB;
	};
	const requestUpdateLanes = () => {
	    return SyncLane;
	};
	const getHighestPriorityLane = (lanes) => {
	    return lanes & -lanes;
	};
	const markRootFinished = (root, lane) => {
	...
	class FiberRootNode {
	    ...
	    current;
	    finishedWork;
	    pendingLanes; // 代表所有未被消费的lane的集合
	    finishedLane; // 代表本次更新消费的lane
	...
	const updateFragment$1 = (returnFiber, current, elements, key, existingChildren) => {
	    let fiber;
	...
	const HookHasEffect = 0b0001; // 当前effect本次更新存在副作用
	...
	const renderWithHooks = (wip, lane) => {
	    // 赋值操作
	...
	const updateFragment = (wip) => {
	    const nextChildren = wip.pendingProps;
	    reconcileChildren(wip, nextChildren);
	    return wip.child;
	};
	const updateFunctionComponent = (wip, renderLane) => {
	    const nextChildren = renderWithHooks(wip, renderLane);
	    reconcileChildren(wip, nextChildren);
	    return wip.child;
	};
	const updateHostRoot = (wip, renderLane) => {
	...
	const commitHookEffectList = (flags, lastEffect, callback) => {
	...
	const commitHookEffectListUnmount = (flags, lastEffect) => {
	...
	const commitHookEffectListDestroy = (flags, lastEffect) => {
	    commitHookEffectList(flags, lastEffect, (effect) => {
	        const destroy = effect.destory;
	        if (typeof destroy === 'function') {
	            destroy();
	...
	const commitHookEffectListCreate = (flags, lastEffect) => {
	    commitHookEffectList(flags, lastEffect, (effect) => {
	...
	const recodeHostChildrenToDelete = (childrenToDelete, unmountFiber) => {
	    // 1. 找到第一个root host节点
	...
	const prepareFreshStack = (root, lane) => {
	...
	const markRootUpdated = (root, lane) => {
	...
	const performSyncWorkOnRoot = (root, lane) => {
	...
	const commitRoot = (root) => {
	    ...
	    // root flags
	    // root subtreeFlags
	    const subtreeHasEffevt = (finishedWork.subtreeFlags & (MutationMask | PassiveMask)) !== NoFlags;
	...
	const flushPassiveEffects = (pendingPassiveEffects) => {
	    // 遍历effect
	...
	const createRoot = () => {
	    ...
	    const childToJSX = (child) => {
	        ...
	        if (Array.isArray(child)) {
	            ...
	            if (children.every((child) => child === 'string' || typeof child === 'number')) {
